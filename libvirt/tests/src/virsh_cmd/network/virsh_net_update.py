...
""" % (net_name, net_name)
    ...
    try:
        ...
        xmldoc = parseString(virtual_net)
        ...
        section_xml = xmldoc.getElementsByTagName(element)[0].toxml()
        ...
        actual_xmldoc = parseString(actual_net_xml)
        logging.info("After net-update, the actual net xml is %s",
                     actual_net_xml)

        if update_command == "delete":
            if actual_xmldoc.getElementsByTagName(net_section):
                raise error.TestFail("The actual net xml is not expected,"
                                     "element still exists" % net_section)
